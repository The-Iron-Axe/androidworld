...
def get_tasks_by_difficulty(difficulty: str) -> list[str]:
  """Returns task names from task_metadata.json matching a difficulty.

  Args:
    difficulty: One of 'easy', 'medium', or 'hard'.

  Returns:
    Sorted list of task names with the given difficulty.
  """
  difficulty = difficulty.lower()
  if difficulty not in ('easy', 'medium', 'hard'):
    raise ValueError(
        f"Invalid difficulty '{difficulty}'. Expected easy, medium, or hard."
    )
  metadata_df = _extract_task_metadata()
  matched = metadata_df.index[metadata_df['difficulty'] == difficulty].tolist()
